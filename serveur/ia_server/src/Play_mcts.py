...
def select_best_card_with_MCTS(player_hand, player_index, all_played_cards, current_trick, scores,duree):
    ...
    print(len(deck.invert_deck))
    state = State(deck, current_trick, scores, player_index)
    card_to_play = mcts.do_action(state,duree)
    return card_to_play